...
// Eliminar registro si se recibe por GET
...
// Guardar si vienen datos por POST
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>{$row['genero']}</td>
                <td>{$row['pais']}</td>
                <td><a href='guardar.php?eliminar={$row['id']}' onclick=\"return confirm('¿Eliminar este registro?');\">Eliminar</a></td>
              </tr>";
    }
